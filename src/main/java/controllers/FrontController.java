...
public class FrontController extends HttpServlet {
     ...
     * （例：パラメータがaction=Enployeeの場合、actions.EnployeeActionオブジェクト）
     * @param request リクエスト
    ...
    private ActionBase getAction(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            // ActionBaseオブジェクトにキャスト（例：actions.EmployeeActionオブジェクト ⇨ actions。ActionBaseオブジェクト）
            ...
            // リクエストパラメータに設定されている"action"の値が不正な場合（例：action=xxxxx 等、該当するActionクラスがない場合）
